...
def rmse(u, u_gt):
    return jnp.sqrt(jnp.mean((u - u_gt)** 2))
...
def _eval3d(apply_fn, params, *test_data):
    ...
    return relative_l2(pred, u_gt), rmse(pred, u_gt)
...
def _eval4d(apply_fn, params, *test_data):
    ...
    return relative_l2(apply_fn(params, t, x, y, z), u_gt), rmse(apply_fn(params, t, x, y, z), u_gt)
...
def setup_eval_function(model, equation):
    ...
    if dim == '2d':
        ...
        if model == 'pinn' and equation == 'navier_stokes3d':
            fn = _eval3d_ns_pinn
        elif model == 'spinn' and equation == 'navier_stokes3d' or model == 'copinn' and equation == 'navier_stokes3d':
            fn = _eval3d_ns_spinn
        else:
            fn = _eval3d
    elif dim == '4d':
        if model == 'pinn':
            fn = _batch_eval4d
        if model == 'spinn' and equation == 'navier_stokes4d' or model == 'copinn' and equation == 'navier_stokes4d':
            fn = _eval_ns4d
        else:
            fn = _eval4d
    elif dim == 'nd':
        if model == 'spinn' or model == 'copinn':
            fn = _evalnd
    else:
