...
# 3d time-dependent diffusion exact u
def diffusion3d_exact_u(t, x, y):
    return (
        0.25 * jnp.exp(-((x - 0.3)**2 + (y - 0.2)**2) / 0.1)
        + 0.4 * jnp.exp(-((x + 0.5)**2 + (y + 0.1)**2) * 15)
        + 0.3 * jnp.exp(-(x**2 + (y + 0.5)**2) * 20)
    )
